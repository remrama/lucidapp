...
import_fname1 = os.path.join(utils.Config.data_directory, "results", "analyze-firstsessions_within.csv")
import_fname2 = os.path.join(utils.Config.data_directory, "results", "analyze-firstsessions_between.csv")
export_fname = os.path.join(utils.Config.data_directory, "results", "analyze-firstsessions.png")


#### Load data.
within_df = pd.read_csv(import_fname1, index_col="subjectCondition")
between_df = pd.read_csv(import_fname2, index_col=["subjectCondition1", "subjectCondition2"])
...
# aesthetics
ylabel = (r"$\Delta$ Lucid Induction Success"
    + "\n(within-subject LDs per night)")
